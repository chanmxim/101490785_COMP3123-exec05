...
- Answer: To avoid a huge code in 1 file, and to separate and group routing logic into designated folders and files. It improves maintainability and reusability 
...
Answer: It starts a server on a port specified in .env file and set the default 8081 if port in .env is unresolved. It gives flexibility in production environments since port inside .env file can be dynamically managed and default port ensures error handling
